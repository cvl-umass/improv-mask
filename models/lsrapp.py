...
def get_trunk(trunk_name, num_inp_feats=3, pretrained=True):
    ...
    #     s4_ch = 24
    #     high_level_ch = 1280
    logger.debug(f"Inside mobilenet. pretrained={pretrained}")
    if trunk_name == 'mobilenetv3_large':
        backbone = MobileNetV3_Large(pretrained=pretrained, num_inp_feats=num_inp_feats)
        s2_ch = 16
        s4_ch = 24
        high_level_ch = 960
    elif trunk_name == 'mobilenetv3_small':
        backbone = MobileNetV3_Small(pretrained=pretrained, num_inp_feats=num_inp_feats)
        s2_ch = 16
        s4_ch = 16
        high_level_ch = 576
    else:
...
class LRASPP(BaseSegmentation):
    ...
    def __init__(self, trunk, use_aspp=False, num_filters=128, num_inp_feats=3, pretrained=True):
        ...
        self.trunk, s2_ch, s4_ch, high_level_ch = get_trunk(trunk_name=trunk, num_inp_feats=num_inp_feats, pretrained=pretrained)
